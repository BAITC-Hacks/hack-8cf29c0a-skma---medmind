...
Кнопка «Новый заказ» добавляет позицию из каталога товаров. Нажмите на позицию,
чтобы открыть карточку с действиями «Редактировать», «Удалить», «Утвердить» и
«Отклонить». Удаление требует подтверждения. Можно менять количество (включая
дробное с точкой или запятой), срочность, обоснование и комментарий. Изменение
возвращает позицию в статус «Ожидает»; исходный прогноз не перезаписывается.
Один товар можно добавить в выбранный расчёт только один раз. Когда расчётов
ещё нет, первый заказ создаёт отдельный набор ручных заказов без вычисления прогноза.

При `VITE_USE_MOCK=false` весь CRUD, утверждение, отклонение и CSV-экспорт используют
API из `VITE_API_BASE_URL` (по умолчанию `/api`). Поиск товаров использует серверный
каталог `/api/input-data/products`; экран «Товары» пока хранит отдельные демоданные.
Для работы нужны запущенный backend и заполненный серверный каталог. В деморежиме
есть дополнительный товар `DEMO-NEW` для проверки создания; изменения сбрасываются
при перезагрузке. Кнопок Excel/PDF в интерфейсе нет.
